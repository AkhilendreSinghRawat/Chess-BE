...
export const getAvailabeMoves = (game, potentialMoves, row, col) => {
  ...
  for (const potentialMove of potentialMoves) {
    ...
    tempGame.makeMove({ currSquare: `${row}-${col}`, newSquare: potentialMove, isTempMove: true });
...
export const getPotentialKingMoves = (game, row, col, isValiationCkeck) => {
  ...
  if (!isValiationCkeck) getPotentialCastlingMoves(game, row, col, moves);

  return { moves, isChecked: checkStatus.isCheck };
};

const getPotentialCastlingMoves = (game, row, col, moves) => {
  if (game.isCheck) return;

  // If king has no castling rights neither left nor right
  if (!game.castlingRights[game.turn].left && !game.castlingRights[game.turn].right) return;

  const { positions } = game.getPositions();

  // If king has left castling right which is longer side
  if (game.castlingRights[game.turn].left) {
    // check if the new sqaure already have a piece
    const oneLeft = row + '-' + (Number(col) - 1);
    const twoLeft = row + '-' + (Number(col) - 2);
    if (!positions[oneLeft] && !positions[twoLeft]) {
      //   now validate oneLeft
      // twoLeft will be automatically validated later
      // temp instance of board for every move.
      const tempGame = game.clone();
      // make a move on temp board.
      tempGame.makeMove({ currSquare: `${row}-${col}`, newSquare: oneLeft, isTempMove: true });
      if (!tempGame.getIsCheck()) {
        moves.push(twoLeft);
      }
    }
  }

  // If king has right castling right which is longer side
  if (game.castlingRights[game.turn].right) {
    // check if the new sqaure already have a piece
    const oneRight = row + '-' + (Number(col) + 1);
    const twoRight = row + '-' + (Number(col) + 2);
    if (!positions[oneRight] && !positions[twoRight]) {
      const tempGame = game.clone();
      // make a move on temp board.
      tempGame.makeMove({ currSquare: `${row}-${col}`, newSquare: oneRight, isTempMove: true });
      if (!tempGame.getIsCheck()) {
        moves.push(twoRight);
      }
    }
...
export const checkValidPromotionPiece = (newSquare, currSpuarePiece, promotionPiece) => {
  if (currSpuarePiece.type !== 'p') {
    console.error('Invalid promotion: current piece is not a pawn', currSpuarePiece.type);
    return false;
  }

  if (
    (currSpuarePiece.color === 'w' && newSquare[0] !== '0') ||
    (currSpuarePiece.color === 'b' && newSquare[0] !== '7')
  ) {
    console.error('Invalid promotion: new square for promotion is not valid', newSquare);
    return false;
  }

  if (!['q', 'r', 'b', 'k'].includes(promotionPiece)) {
    console.error('Invalid promotion: promotion piece is not a valid', promotionPiece);
    return false;
  }
